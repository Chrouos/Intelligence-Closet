...
class ArduinoController:
    ...
    def takeClothes_single(self, position):
        ser = serial.Serial(self.COM_PORT, self.BAUD_RATES)
        mcu_feedback = 'Reset'
        instruction_send_done = False
        try:
    ...
    def put_EntrancePositionZero(self, position):
        ser = serial.Serial(self.COM_PORT, self.BAUD_RATES)
        mcu_feedback = 'Reset'
        instruction_send_done = False
        try:
            while mcu_feedback != "Done":
                ...
                if mcu_feedback != 'Reset' and instruction_send_done == False:
                    ser.write(b'Put_Entrance_Position_Zero\n')     # 訊息必須是位元組類型
                    sleep(0.5)                          # 暫停0.5秒，再執行底下接收回應訊息的迴圈
                    mcu_feedback = "Doing"
                    print('[Put_Entrance_Position_Zero] 入口到位置後歸零...')
                    instruction_send_done = True

                while ser.in_waiting:
                    mcu_feedback = ser.readline().decode()  # 接收回應訊息並解碼
                    mcu_feedback = mcu_feedback.replace("\n", "")
                    print('[Put_Entrance_Position_Zero] 回應：', mcu_feedback)
                    
                    if "please_input_str_position" in mcu_feedback:
                        msg = str(position) + '\n'
                        ser.write(msg.encode())
                        sleep(0.5)
                        print("[Put_Entrance_Position_Zero] 收到存放指令，輸入模塊位置為:", position)
...
Put_Entrance_Position_Zero: put_entrance_position_zero
