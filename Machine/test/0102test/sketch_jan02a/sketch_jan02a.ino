...
LiquidCrystal_I2C lcd(
    0x27, 16,
    4); // I2C位址，默認為0x27或0x3F，依據背板的晶片不同而有差異，16、2為LCD顯示器大小。
// ----------------------------- 控制腳位 start ----------------------------- //
//紅外遙控器
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    // //接收訊號，以16進位型式輸出到監控視窗 Serial.println("------------");
...
// -----------------圓盤----------------- //
// 確認按鈕狀況
int checkTheBtnStatus(const int button, int &buttonState, int &buttonLastState,
                      long &buttonlastDebounceTime, long delayTime) {
...
void discRotate_withTimes(int times) {
  ...
  Serial.println("discRotate_withTimes have to rotate " + String(times) +
                 " Times");
  ...
  while (now_times != times) {
    ...
    if (millis() - temp_time > 500) {
      if (checkTheBtnStatus(discButton, discButtonState, discButtonLastState,
                            discButtonlastDebounceTime,
                            globalDelayTime) == true) {
        disc_start = false;
        now_times++;
      }
    }
