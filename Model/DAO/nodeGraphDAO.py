...
class NodeGraphDAO:
    ...
    def create(self, nodeGraph_dict):
        ...
        if(nodeGraph_dict['CategoryId'] == 1):
            execute_str = "INSERT INTO intelligence_closet.dbo.node_graph (UpperId, LowerId, CreateTime, ModifyTime, UserLike) SELECT " \
                + "{0}, Id, GETDATE(), GETDATE(), 0 FROM intelligence_closet.dbo.v_clothes_node WHERE CategoryId = 2;".format(
                    nodeGraph_dict['ClothesNodeLastId'])
        
        elif(nodeGraph_dict['CategoryId'] == 2):
            execute_str = "INSERT INTO intelligence_closet.dbo.node_graph (UpperId, LowerId, CreateTime, ModifyTime, UserLike) SELECT " \
                + "Id, {0}, GETDATE(), GETDATE(), 0 FROM intelligence_closet.dbo.v_clothes_node WHERE CategoryId = 1;".format(
                    nodeGraph_dict['ClothesNodeLastId'])
        else:
